...
void wujiang::Kanshoupai()
{
  select_shoupai(S_VIEW,shoupai,shoupaishu);
}
void wujiang::Kanzhuangbei()
{
	show_equipment(*this);
}
void wujiang::Kanpandingpai()
{
	show_judgement(*this);
}
...
void Qishishoupai(wujiang *w,pai A[104],int *x,pai B[104],int*y)
{
 ...
 int m;
 if((*w).juese)printf("玩家摸4张牌\n");
 else printf("电脑摸4张牌\n");
 if((*w).juese)
 {
	select_shoupai(S_VIEW,w->shoupai,w->shoupaishu);
 }
 printf("还剩%d张牌\n",*x);
}
...
int Xuanpai(wujiang*w,int t)
{
 ...
 if((*w).juese)
 {
  ...
  {
      printf("出牌请选择手牌\nEXIT结束出牌\n");
      x=select_shoupai(S_SELECT,w->shoupai,w->shoupaishu);
 ...
 case 205:
  ...
  if((*w1).juese)
  {
   ...
   else
   {
    ...
    {
        printf("请选择【杀】的目标！");
		printf("提示：【杀】的目标必须在【借刀杀人】的目标的攻击范围之内！");
		printf("输入目标前的编号，以回车结束！");
		printf("0      电脑1\n");
        y=S();
     if(y!=0)
...
int Wuxie(pai *p,wujiang*w1,wujiang*w2,pai qipaidui[104],int*qipaishu,int a)
{
 ...
 if((*w1).juese)
 {
  ...
  for(;;)
  {
   printf("如果要使用【无懈可击】则选择手牌\n不需要则EXIT\n");
   x=select_shoupai(S_SELECT,w2->shoupai,w2->shoupaishu);
   if(x==-1)
   {
  ...
  else if(x==0&&((*w1).zhuangbei[0].leixing==304))
  {
   for(;;)
   {
    ...
    if(x==1)
    {
     ...
     {
      printf("请对电脑使用一张【杀】！\nEXIT放弃出【杀】\n");
      x=select_shoupai(S_SELECT,w1->shoupai,w1->shoupaishu);
      if(x==-1)
      {
       return 0;
       break;
  ...
  for(j=1;j<=q;j++)
  {
   for(;;)
   {
    if(juese)
    {
              ...
              x=select_shoupai(S_VIEW,shoupai,*shoupaishu);
...
int Huihe(pai A[104],pai B[104],int *x,int *y,pai yuanshipaidui[2][4][13],wujiang *w1,wujiang *w2)
{
 ...
 if((*w1).pandingshu>0)
 {
  ...
  for(;(*w1).pandingshu;)
  {
   ...
   {
   case 251:printf("【乐不思蜀】判定……\n"); break;
   case 252:printf("【闪电】判定……\n");break;
   default:printf("【？？】判定……\n");break;
   }
   j=0;
